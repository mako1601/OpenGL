...
#include "Core/Log.h"
#include "Core/Rendering/OpenGL/ShaderProgram.h"
#include "Core/Rendering/OpenGL/VertexBuffer.h"
#include "Core/Rendering/OpenGL/VertexArray.h"
#include "Core/Rendering/OpenGL/IndexBuffer.h"
...
//       x      y     z       r     b     g
    ...
    int Window::init() {
        ...
        if (!s_GLFW_initialized) {
            if (!glfwInit()) {
                LOG_CRITTICAL("Can't initialize GLFW!");
                return -1;
            }
        ...
        if (!m_pWindow) {
            LOG_CRITTICAL("Can't create window {0}", m_data.title);
            glfwTerminate();
            return -2;
        }
        ...
        if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
            LOG_CRITTICAL("Failed to initialize GLAD");
            return -3;
        }
        ...
        p_positions_colors_vbo = std::make_unique<VertexBuffer>(positions_colors, sizeof(positions_colors), buffer_layout);
